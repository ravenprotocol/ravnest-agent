...
class Phi3SplitSpec(LlamaSplitSpec):
    """Phi-3 uses the same architecture as Llama (embed_tokens, layers, norm)."""
...
class Phi3ForCausalLMSplitSpec(Phi3SplitSpec):
    def get_stage_layers(self):
...
class PhiSplitSpec(Phi3SplitSpec):
    """Phi-1/Phi-2 (PhiModel)."""

    def get_stage_layers(self):
        if self.model.__class__.__name__ == "PhiModel":
            module = self.model
        else:
            module = self.model.model
        return self._get_layers_from_module(module)
...
class PhiForCausalLMSplitSpec(PhiSplitSpec):
    def get_stage_layers(self):
        held_layers = super().get_stage_layers()
        from ...strings import NodeTypes
        if self.node_type == NodeTypes.LEAF:
